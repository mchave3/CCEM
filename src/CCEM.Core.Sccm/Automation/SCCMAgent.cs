...
namespace CCEM.Core.Sccm.Automation
{
    ...
    public class SCCMAgent : IDisposable
    {
        ...
        /// <summary>
        /// Connect to a remote SCCM Agent by using WSMan (integrated auth).
        /// </summary>
        /// <param name="hostname">target computername</param>
        /// <param name="wsManPort">WSManagement Port (Default = 5985)</param>
        /// <param name="connect">If false, only prepares the connection and requires calling <see cref="connect"/>.</param>
        /// <param name="encryption">Enable encryption (HTTPS)</param>
        public SCCMAgent(string hostname, int wsManPort, bool connect, bool encryption)
        {
            Initialize(hostname, credential: null, wsManPort, connect, encryption);
        }
